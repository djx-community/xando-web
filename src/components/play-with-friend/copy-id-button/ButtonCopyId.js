...
function ButtonCopyId({ id }) {
    ...
    return (
       ...
       border-t-4 border-slate-500 hover:border-blue-500 rounded flex gap-1">
                <input type='text' placeholder='Your ID' value={id} disabled
                ...
                <DocumentDuplicateIcon className='text-white shadow h-10 bg-inherit hover:bg-sky-700 hover:shadow-white'
                    onClick={() =>
                        handleButtonClick()
                    } />
            </div>) : (
            <button className="bg-sky-900 text-white text-2xl 
